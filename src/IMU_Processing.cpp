...
{
  SPDLOG_WARN("Online Gravity Estimation Disabled !!!!!");
  gravity_est_en = false;
}

void ImuProcess::disable_bias_est()
{
  SPDLOG_WARN("Bias Estimation Disabled !!!!!");
  ba_bg_est_en = false;
}

void ImuProcess::disable_exposure_est()
{
  SPDLOG_WARN("Online Time Offset Estimation Disabled !!!!!");
...
void ImuProcess::UndistortPcl(LidarMeasureGroup &lidar_meas, StatesGroup &state_inout, PointCloudXYZI &pcl_out)
{
  ...
  case VIO:
    ...
    for (int i = 0; i < v_imu.size() - 1; i++)
    {
      ...
      // imu_time = head_time - first_lidar_time;
      ...
      // cout<<setw(20)<<"offset_t: "<<offs_t<<"tail->header.stamp.seconds():
